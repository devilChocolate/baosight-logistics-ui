    ...
    methods:{
        ...
        setSortRows(flag){
            if(!this.sortAxiosEnabled&&Object.keys(this.sortParam).length){
                ...
                const list=JSON.parse(JSON.stringify(this.list)).sort((a,b)=>{
                    let _prop=this.sourceCol[prop].propTitle||this.sourceCol[prop].prop;
        ...
        getPageInfo(page){
            ...
            this.getListInfo().then(({result,msg})=>{
                this.list.splice(0,this.list.length,...result.data.slice(0,this.pageSize));
        ...
        getAxiosParam(){
            ...
            o=Object.assign(o,isFunction(this.axiosMap)?this.axiosMap('send',this.axiosParam,this.getAxiosIndex.read):this.axiosParam);
            return o;
        },
        /*
    	 * 获取列表数据
                ...
                if(this.dataSource){
                    resolve({
                        result:{
                            ...
                            data:JSON.parse(JSON.stringify(this._dataSource)).slice((this.currentPage-1)*this.pageSize,this.currentPage*this.pageSize)
                        }
                    });
                }else{
                    this.$http({
                        method:this.axiosType,
                        url:isPlainObject(this.axiosUrl)?this.axiosUrl.read:this.axiosUrl,
                        data:{
                            ...this.getAxiosParam(),
        ...
        formatList(data){
            return data.map((item,index)=>{
                ...
                item.checked=this.historyCheckFilterId?!!this.historyChecks[this._getRowFilterId(item)]:false;
                item.addabled=false;
                item.showabled=true;
