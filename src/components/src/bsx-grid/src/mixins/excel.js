...
export default {
    ...
    methods:{
        ...
        exportAllPage2Excel(){
            if(this.dataSource){
                this.export2Excel(this._dataSource.map((row,index)=>{
                ...
                this.$http({
                    ...
                    this.export2Excel(res.list.map((row,index)=>{
                        return this._mergeDependRowData(row,index);
                    }));
                }).catch(({msg})=>{
                    loading&&loading.close();
    	 ...
    	 */
        export2Excel(rows){
            const cols=Object.keys(this.sourceCol).map(key=>{
                return this.sourceCol[key];
            }).filter(col=>{
                return col.prop&&!col.unProp&&col.exportable&&col.type!='upload'&&col.type!='custom';
            });
            ...
            rows=rows.map((row,index)=>{
                let _row=cols.map(col=>{
                    let value=value=col.propTitle?row[col.propTitle]:row[col.prop];
                    if(col.type=='calendar'){
                        value=value.split(col.multipleSeparate).map(date=>{
            ...
            export_json_to_excel({
                header:tHeader,
                data:rows,
                filename:this.exportExcelFileName||`grid-${this.formatDate(new Date(),'YYYY-MM-DD hh:mm:ss').replace(/(\s+)|-|:/gm,'')}-${createRandomUid()}`
            });
        }
    },
    components:{
