    ...
    methods:{
        ...
        getExpandChildGrid(index){
            ...
            if(expandComponent){
                if(index!=undefined){
                    ...
                    const expandGrid=expandComponent.map(item=>{
                        if(isFunction(item._getGridComponent)){
        ...
        getRowCellComponent(index,isAdd=false){
            ...
            Object.keys(this.sourceCol).map(key=>{
                ...
                if(cell){
                    ...
                    if(cell&&(cell=cell.$refs&&cell.$refs.formItem)&&cell.fetchValid){
                        cells[key]={
        ...
        _updateRowsData(rows){
            return new Promise((resolve,reject)=>{
                ...
                if(flag&&this.dataSource){
                    this._dataSource.splice(this.pageSize*(this.currentPage-1),0,...Object.keys(rows.adds||{}).map(index=>{
                        return {
                            ...rows.adds[index],
                            editabled:false,
                            checked:false,
        ...
        _removeRowsData(rows){
            Object.keys(rows||{}).forEach((index,i)=>{
                ...
                if(this.dataSource){
                    this._dataSource.splice(this.pageSize*(this.currentPage-1)+Number(index)-i,1);
                    this.total=this._dataSource.length;
                };
        ...
        _resetScrollLeft(left=0){
            ...
            body_center_scroll&&body_center_scroll.scrollTo({
                left:left
            });
        ...
        _mergeDependRowData(row,index=0){
            Object.keys(this.sourceCol).map(key=>{
                let col=this.sourceCol[key];
                if(col.prop&&col.prop!=this.checkType&&!row.hasOwnProperty(col.prop)&&!col.unProp){
                    ...
                    if(row[col.prop]&&row[col.prop]==new Array(row[col.prop].split(col.multipleSeparate).length).join(col.multipleSeparate)){
                        row[col.prop]='';
                    };
                    if(col.propTitle&&row[col.propTitle]&&row[col.propTitle]==new Array(row[col.propTitle].split(col.multipleSeparate).length).join(col.multipleSeparate)){
                        row[col.propTitle]='';
                    };
                };
            });
        ...
        _setDependVal(row,col,type,prop){
            const dependVal=isArray(row[prop])?row[prop]:row[prop].split(col.multiple);
            const depend=isArray(col[type])?col[type]:col[type].split(',');
            depend.map((key,i)=>{
            ...
            return new Promise((resolve,reject)=>{
                if(this.dataSource){
                    ...
                    this.$http({
                        ...
                        url:isPlainObject(this.axiosUrl)?this.axiosUrl[type]:this.axiosUrl,
                        data:isFunction(this.axiosMap)?this.axiosMap('send',param,this.getAxiosIndex[type]):param,
                        noLoading:true
                    }).then(res=>{
                        res=res.data;
                    ...
                    //     name:isPlainObject(this.axiosName)?this.axiosName[type]:this.axiosName,
                    //     data:isFunction(this.axiosMap)?this.axiosMap('send',param,this.getAxiosIndex[type]):param,
                    //     loading:{
                    //         enabled:true,
                    //         size:35,
    	 ...
    	 */
        _getRowFilterId(row){
            return isFunction(this.historyCheckFilterId)?this.historyCheckFilterId(row):row[this.historyCheckFilterId];
        }
    }
};
