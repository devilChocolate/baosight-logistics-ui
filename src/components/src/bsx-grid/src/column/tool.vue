...
<template>
    <div class="app_grid-column-container flex-row align-stretch">
        <div class="app_grid-column-menu">
            <div class="app_grid-column-menu-item"
                :class="{active:index==activeIndex}"
                :key="index"
                v-for="(title,index,i) in getMenuKey"
                @click="change(index)">
                <i class="iconfont icon" :class="getIcon[index]"></i>
                <span>{{title}}</span>
            </div>
        </div>
        <div class="app_grid-column-content"
            v-if="activeIndex==0&&getFilterFlag"
            @change-save="clear">
        ...
        <div class="app_grid-column-content"
            v-else-if="!isExpand&&activeIndex==2"
            @change-save="clear">
