...
<template>
    <div class="app_grid-column-info">
        <bsx-scrollbar class="app_grid-column-main" :margin="8" :space="5" :thumb-size="4">
            <el-checkbox-group v-model="modelVal" class="flex-col app_grid-column-group">
                <el-checkbox class="flex-row align-center-row"
                    :label="item.value"
                    :key="i"
    ...
    methods:{
        ...
        reset(val){
            val=(val||this.list).map((row,i)=>{
                const cell=this.getGrid.getRowCellComponent(i)[this.col.prop].component;
            ...
            this.modelVal=this.modelList.filter(item=>{
                return item.checked;
            }).map(item=>{
                return item.value;
            });
