  ...
  <transition name="app_result" v-on:after-leave="afterLeave">
    <div class="app_result-container flex-col" v-show="visible">
      ...
      >
        <div class="relative">
          <img
            v-if="getImageEnabled"
            :src="getImageUrl"
            class="app_result-img"
          />
          <template v-if="mini">
            <p class="app_result-text">
              {{ msg
              }}<span
                class="app_result-text-tip"
                @click.stop="handleRefresh"
                v-if="buttonable"
                ><i class="iconfont app_result-icon icon-shuaxin1"></i
                ><a class="app_result-button-mini" href="javascript:;"
                  >点击重新刷新</a
                ></span
              >
            </p>
          ...
          <template v-else>
            ...
            <a
              class="app_result-button flex-row align-center"
              v-if="buttonable"
              href="javascript:;"
              @click.stop="handleRefresh"
              ><i class="iconfont app_result-icon icon-shuaxin1"></i
