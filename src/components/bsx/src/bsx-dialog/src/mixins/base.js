  ...
  methods: {
    ...
    handleButtonClick(item) {
      if (isFunction(item.callback)) {
        item.callback.apply(
          this,
          [this.$refs.container, this.close, this.getInstance].concat(
            this.args || []
          )
        );
      } else {
    ...
    handleScroll(data) {
      this.scroll
        ? this.scroll(data, this.getInstance)
        : this.$emit("bsx-dialog-scroll", {
            data: data,
    ...
    HandleMousemoveDocument(e) {
      ...
      this.handleDrag
        ? this.handleDrag(this.getInstance)
        : this.$emit("bsx-dialog-drag", {
            instance: this.getInstance,
          });
    },
    /*
