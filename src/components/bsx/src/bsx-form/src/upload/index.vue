...
<template>
  <div
    class="bsx-form-box relative flex-row align-stretch align-start-row"
    :class="{ 'no-gridable': !gridable }"
  >
    <div
      class="bsx-form-label flex-row"
      :class="{ 'align-end-row': !!labelWidth }"
      :style="getLabelStyle"
      v-if="showLabel"
    >
      <i v-if="showRequiredStar" class="star">*</i>{{ label }}：
    </div>
    <div class="flex1 relative bsx-form-upload">
      <template v-if="editabled">
        ...
        >
          <el-button
            @click.prevent="handleClickUploadButton"
            :disabled="disabled"
            :size="size"
            slot="trigger"
            type="primary"
            >点击上传</el-button
          >
          <p slot="tip" class="bsx-form-upload-tip flex1 mar-left-5">
            {{ getTipText }}
          </p>
        </el-upload>
        <div
          class="bsx-form-upload-message"
      ...
      <div class="bsx-form-upload-list" v-if="files && files.length">
        <div
          class="bsx-form-upload-item flex-row align-start-row"
          :class="{
            editable: editabled && !disabled,
            'p-r': editabled && !item.loading,
          }"
        ...
        >
          ...
          <p class="bsx-form-upload-name flex1">
            <a
              href="javascript:;"
              v-if="editabled"
              @click="handleOnPreview(item)"
              >{{ item.name }}.{{ item.type }}</a
            >
            <a :href="item.src" target="_blank" v-else
              >{{ item.name }}.{{ item.type }}</a
            >
          </p>
          <template v-if="item.loading">
            ...
            <span class="mar-left-5 bsx-form-upload-loading-text"
              >{{ item.percent }}%</span
            >
          </template>
          <template v-else-if="editabled">
            ...
            <i
              class="el-input__icon el-icon-delete mf m2"
...
import { isArray, isFunction, isPlainObject, extend } from "@/utils/tool";
...
export default {
  ...
  computed: {
    ...
    getAction() {
      return isPlainObject(this.axiosUrl)
        ? this.axiosUrl.upload
        : this.axiosUrl;
    },
    getAxiosParam() {
      return isFunction(this.axiosMap)
        ? this.axiosMap("send", this.axiosParam, this.getAxiosIndex.upload)
        : this.axiosParam;
    },
    getTipText() {
      let str = "";
    ...
    isDisabled() {
      return (
        this.disabled ||
        (!this.disabled && this.files.length >= this.multipleLimit)
      );
    },
    getLimit() {
  ...
  methods: {
    ...
    handleOnSuccess(response, file, fileList) {
      ...
      if (isFunction(this.axiosMap)) {
        response = this.axiosMap(
          "response",
          response,
          this.getAxiosIndex.upload
        );
      }
      for (let i = 0; i < this.files.length; i++) {
