...
<template>
  <div
    class="app_grid-cell flex-row"
    v-if="!col.isExpand && col.prop !== 'radio'"
  >
    <template v-if="col.prop != checkType || col.isIndex">
      <div
        class="flex1 flex-row"
        :class="[
          {
            center: 'align-center',
            left: 'align-start-row',
            right: 'align-end-row',
          }[col.align],
        ]"
      ...
      >
        ...
        <div
          class="app_grid-tool-btn app_grid-sort"
          v-if="col.sortable && (col.type != 'upload' || col.type != 'custom')"
          :class="{
            up: sort[col.prop] && sort[col.prop] == 'asc',
            down: sort[col.prop] && sort[col.prop] == 'desc',
          }"
        ></div>
      </div>
      <template v-if="col.toolEnabled && col.prop && !col.unProp">
        <div
          class="flex-row align-center align-stretch mar-left-5 app_grid-tool-drop"
        >
          <el-popover
