    ...
    >
      <div class="flex-col app_grid-column-group">
        <template v-for="(cols, type, i) in modelList" v-if="cols.length">
          <transition-group
            class="app_grid-column-rows flex-col"
            name="flip-list"
            tag="div"
            :key="i"
          >
            ...
            >
              <div class="flex-row align-between-row el-checkbox__label">
                ...
                <div class="flex-row align-center">
                  <i
                    class="iconfont icon-shangjiantou1"
                    :class="{ disabled: j == 0 }"
                    @click="handleUp(item, j, cols)"
                    title="上移"
                  ></i>
                  <i
                    class="iconfont icon-xiajiantou1"
                    :class="{ disabled: j == cols.length - 1 }"
                    @click="handleDown(item, j, cols)"
                    title="下移"
                  ></i>
                </div>
              </div>
