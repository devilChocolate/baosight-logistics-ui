 ...
 */
import { isFunction } from '@utils/tool.js'
import {
  addResizeListener,
  removeResizeListener,
} from "@utils/resize.js";
...
export default {
  ...
  methods: {
    ...
    handleRowExpand(row, index) {
      if (row.expanding) {
        return this.showMessages("正在加载中，请勿重复点击！");
      }
      if (row.expand) {
    ...
    handleScroll(
      ...
      if (direction == "head") {
        "left|center|right".split("|").map((item) => {
          if (item != type) {
            this._fetchScroll(
              this.$refs[`body_${item}_scroll`],
              "top",
              scrollTop
            );
          } else if (type == "center") {
            this._fetchScroll(
              this.$refs[`${direction}_${item}_scroll`],
              "left",
              scrollLeft
            );
        ...
        this._fetchScroll(
          this.$refs[`${direction}_${type}_scroll`],
          "left",
          scrollLeft
        );
        !this.isBorder && this._getScrollState(scrollLeft, horizontal);
      ...
      if (this.isChildGrid) {
        if (this.list[index].checked) {
          if (this._parentGrid && this._parentGrid.checkType) {
            this._parentGrid.handleCellChange(
              this._expandChildIndex,
              {
