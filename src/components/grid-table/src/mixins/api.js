...
export default {
  methods: {
        ...
        if (flag) {
          ...
          Object.keys(o).forEach((locked) => {
            o[locked] = o[locked]
              ...
              .map((item, i) => {
                if (
                  this.sourceCol[item.prop] &&
                  !this.sourceCol[item.prop].unSortIndex
                ) {
    ...
    deleteCheckedRows() {
      return new Promise((resolve, reject) => {
        ...
        if (this._hasRows(rows) || !!Object.keys(this.historyChecks).length) {
          ...
          let paramRows = this._Object2Array(disAddlRows),
            hisRows = [];
    ...
    getCellValue({ index, prop, isAdd = false, isActual = true }) {
      const row = isActual
        ? this.getRowValue(index, isAdd)
        : this._getRow(index, isAdd);
      return row && row[prop];
    },
    /*
     * 获取行数据
        ...
        if (row.editabled) {
          ...
          Object.keys(cells).forEach((key) => {
            ...
            if (isArray(newRow[col.prop]) && col.type != "upload") {
              ...
              if (
                newRow[col.prop] ==
                new Array(newRow[col.prop].length).join(col.multipleSeparate)
              ) {
                newRow[col.prop] = "";
              }
            }
            if (isArray(newRow[col.propTitle])) {
              newRow[col.propTitle] = newRow[col.propTitle].join(
                col.multipleSeparate
              );
              if (
                newRow[col.propTitle] ==
                new Array(newRow[col.propTitle].length).join(
                  col.multipleSeparate
                )
              ) {
                newRow[col.propTitle] = "";
    ...
    async fetchVaildRow(index, isAdd, isScrollTo = true) {
      ...
      for (let key in cells) {
        ...
        if (!_flag && isScrollTo && !isPos) {
          ...
          const body_center_rect =
            body_center_scroll.$el.getBoundingClientRect();
          this._resetScrollLeft(
            rect.left -
              body_center_rect.left +
              body_center_scroll.wrap.scrollLeft -
              5
          );
          this._resetScrollTop(
            rect.top -
              body_center_rect.top +
              body_center_scroll.wrap.scrollTop -
              35
          );
        }
      }
