      ...
      <tr>
        <template v-for="(col, i) in cols">
          <th
            ...
            :class="{
              highter: col.highlightable,
              expand: col.isExpand && checkType == 'radio',
            }"
            :align="col.align"
            :width="col.width"
          >
    ...
    <tbody v-else-if="type == 'body'">
      <template v-if="adds && adds.length">
        <tr
          ...
          :class="{
            disabled: row.disabled && checkType,
            isAdd: true,
            child: isChild,
          }"
        ...
        >
          <template v-for="(col, i) in cols">
            <td
              ...
              :width="col.width"
            >
              <div v-if="col.isIndex" class="bsx-form-cell">
                {{ getIndexValue(j, false, true) }}
              </div>
              <slot
      ...
      <template v-for="(row, j) in totalRows">
        <tr
          ...
          :class="{
            even: j < rows.length && getStripeFlag(j, row),
            disabled: row.disabled && checkType,
            checked: clickRowIndex == j,
            child: isChild,
            redRow: row.red,
            blueRow: row.blue,
          }"
        ...
        >
          <template v-for="(col, i) in cols">
            <td
              ...
              class="app_grid-table-td"
              :class="{
                borT: getSumFlag && getBorderFlag(j),
                fontB: getSumFlag && j >= rows.length,
                highter: col.highlightable,
                expand: col.isExpand,
              }"
              :align="col.align"
            ...
            >
              ...
              >
                ...
                <i
                  v-else
                  class="icon iconfont icon-jiantou-you-cuxiantiao"
                  :class="{ active: row.expand }"
                />
              </div>
              <slot
                ...
                <template v-else>
                  ...
                  <template v-else-if="sumable && checkSumable">
                    <span class="text-overflow">
                      <slot
                        v-if="j == totalRows.length - 2"
                        name="sum"
                        text="合计"
                        :index="i"
                      />
                      <slot
                        v-if="j == totalRows.length - 1"
                        name="sum"
                        text="选中合计"
                        :index="i"
                      />
  ...
  methods: {
    ...
    getSumSlotFlag(index, col) {
      ...
      if (
        this.checkSumable &&
        col.checkSumable &&
        index == this.totalRows.length - n + (this.sumable ? 1 : 0)
      ) {
        return true;
      }
    },
    ...
    getBorderFlag(index) {
      ...
      return (
        index >= this.totalRows.length - n - 1 &&
        index != this.totalRows.length - 1
      );
    },
    /*
     * 获取序号
    ...
    getStripeFlag(index, row) {
      return (
        this.stripe && !row.unstripe && this.getIndexValue(index, true) % 2 == 0
      );
    },
  },
};
