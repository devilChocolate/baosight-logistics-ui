 ...
 */
import bsxFormItem from "../../../bsxFormItem.vue";
import { isFunction, extend, isBoolean, isArray } from "@utils/tool.js";
...
export default {
  ...
  methods: {
    ...
    handleCellChange(val) {
      this.col.instance &&
        this.col.instance.handleCellChange(this.index, val, this.col, this.row);
    },
    /*
     * 自定义图标单击
     */
    handleCellInputIconClick(val) {
      this.col.instance &&
        this.col.instance.handleCellInputIconClick(
          this.index,
          val,
          this.col,
          this.row
    ...
    if (this.col.type != "custom") {
      component = h(bsxFormItem, {
        props: {
          ...
          value:
            isArray(this.value) || isBoolean(this.value)
              ? this.value
              : `${this.value}`,
          code: this.row[this.col.propCode],
          ...
          format: isFunction(this.col.format)
            ? (val) => this.col.format(this.index, val, this.col, this.row)
            : this.col.format,
          ...
          axiosMap: isFunction(this.col.axiosMap)
            ? (function (that) {
                return function () {
                  return that.col.axiosMap.apply(
                    null,
                    [].slice.call(arguments, 0).concat(that.col, that.index)
                  );
                };
              })(this)
            : this.col.axiosMap,
